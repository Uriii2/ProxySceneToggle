...
using System.Threading; // Подключаем это вместо System.Timers
...
class ProxyToggleApp : ApplicationContext{
	...
	void OnTrayClick(object sender, MouseEventArgs e)
    {
		...
		//if (e.Button == MouseButtons.Left) UpdateUI();
	}
    ...
    public static byte[] GetCurrentBytes()
    {
		...
		string valueName = "DefaultConnectionSettings";          // Имя значения
	...
	{		
		...
		//Ю+++ раскоментировать, если нужно всплывающее окно из трея!
	...
	//Ю+++ если вручную не выключалось, но по каким-то причинам система сценарий отключила, то заново включаем. 
	...
	/* 
		Алиса что изменено:
			...
			Проблема не только в модификаторе OnLoad, но и в том, что ApplicationContext 
			(от которого наследуется ProxyToggleApp) не имеет метода OnLoad — а значит, и 
			base.OnLoad(e) не скомпилируется. Проще всего убрать OnLoad и вызывать StartPeriodicCheck() прямо из 
			конструктора.
							Удалён OnLoad — ApplicationContext не предоставляет такого метода. Вместо 
			него StartPeriodicCheck() вызывается из конструктора.
							Раскомментирован private Timer _timer — без него StartPeriodicCheck() не 
			скомпилируется.
					
			Я:
				ваш исправленный код также отработал с ошибкой, посмотрите: (15,13): error 
				CS0104: Неоднозначная ссылка "Timer" между "System.Windows.Forms.Timer" и "
